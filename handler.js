...
—◉ *Contacto para apelaciones:* wa.me/51917311570
╚═════════════════════════╝
               `.trim();
              m.reply(messageText);
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Lucas👑;;;\nFN:Lucas 👑\nORG:Lucas 👑\nTITLE:\nitem1.TEL;waid=51917311570:+51 917 311 570\nitem1.X-ABLabel:Lucas 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:Lucas 👑\nEND:VCARD`;
        await this.sendMessage(nk.from, {contacts: {displayName: 'Lucas 👑', contacts: [{vcard}]}}, {quoted: callmsg});
